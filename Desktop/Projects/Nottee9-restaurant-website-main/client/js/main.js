...
// Ensure the page always starts at the top on load/reload
if ('scrollRestoration' in history) {
  try { history.scrollRestoration = 'manual'; } catch (e) { /* ignore */ }
}
window.addEventListener('pageshow', (ev) => { if (ev.persisted) window.scrollTo(0,0); });
window.addEventListener('beforeunload', () => { window.scrollTo(0,0); });

/* ── STATE ───────────────────────────────────────────────────── */
let allMenuItems = [];
let menuFilters  = { cat: 'all', type: 'all', cuisine: 'all' };
let menuPreview  = true; // show two-row preview initially
let menuSearchQuery = '';

/* ── LOADER ──────────────────────────────────────────────────── */
window.addEventListener('load', () => {
  // Force scroll to top when the page finishes loading
...
async function loadSpecials() {
  try {
    ...
    // Fallback: if specials endpoint returns empty, try fetching all menu and filter
    if (!data.success || !data.data.length) {
      const allRes = await fetch(`${API}/menu`);
      const allData = await allRes.json();
      data = { success: allData.success, data: (allData.data || []).filter(i => i.isSpecial) };
    }
    ...
    // Re-observe new special cards for scroll reveal
    grid.querySelectorAll('.rv').forEach((el, i) => { el.style.transitionDelay = `${i * 40}ms`; revObs.observe(el); });
  } catch {
...
function renderMenu() {
  ...
  const previewBtn = document.getElementById('toggleMenuBtn');

  let filtered = allMenuItems.filter(d => {
    const catOk     = menuFilters.cat === 'all' || (menuFilters.cat === 'offers' ? d.isOffer : d.category === menuFilters.cat);
  ...
  // Apply search filtering (name, description, cuisine, category)
  if (menuSearchQuery) {
  ...
  // If preview mode is active, limit items to two rows based on current columns
  ...
  if (menuPreview) {
    ...
    const limit = cols * 2; // two rows
    itemsToShow = filtered.slice(0, limit);
    // Show or hide preview button
    if (previewBtn) {
      if (filtered.length > limit) { previewBtn.style.display = ''; previewBtn.textContent = 'See more'; }
      else { previewBtn.style.display = 'none'; }
    }
  } else {
    if (previewBtn) { previewBtn.style.display = ''; previewBtn.textContent = 'See less'; }
  }

  grid.innerHTML = itemsToShow.map(d => buildCard(d)).join('');
...
// Toggle between preview (two rows) and full menu
...
// Public menu search (debounced)
function debounce(fn, wait = 220) {
  let t;
  return (...args) => { clearTimeout(t); t = setTimeout(() => fn(...args), wait); };
}

function applyMenuSearch(q) {
  menuSearchQuery = (q || '').toString().trim().toLowerCase();
  renderMenu();
}

// Attach search input if present
const publicSearchInput = document.getElementById('menuSearchPublic');
if (publicSearchInput) {
  publicSearchInput.addEventListener('input', debounce(e => applyMenuSearch(e.target.value)));
}
...
document.getElementById('conForm')?.addEventListener('submit', async e => {
  ...
  // If you want to use a zero-server form provider (Formspree/Getform),
  // set window.FORM_PROVIDER_URL in the page (see README). Example:
  // <script>window.FORM_PROVIDER_URL = 'https://formspree.io/f/your-id'</script>
  const FORM_PROVIDER = window.FORM_PROVIDER_URL || '';

  if (FORM_PROVIDER) {
    // Send as multipart/form-data to the provider
    const fd = new FormData(form);
    try {
      const res = await fetch(FORM_PROVIDER, { method: 'POST', body: fd, headers: { 'Accept': 'application/json' } });
      if (res.ok) {
        msg.className = 'mt-3 p-3 rounded text-sm text-center bg-[#00ff9c]/10 border border-[#00ff9c]/30 text-[#00ff9c]';
        ...
        msg.className = 'mt-3 p-3 rounded text-sm text-center bg-[#ff2b2b]/10 border border-[#ff2b2b]/30 text-[#ff2b2b]';
        msg.textContent = data.error || 'Failed to send message. Please try again later.';
        showToast('✖️ Failed to send message', true);
      }
    } catch (err) {
      msg.className = 'mt-3 p-3 rounded text-sm text-center bg-[#ff2b2b]/10 border border-[#ff2b2b]/30 text-[#ff2b2b]';
      msg.textContent = 'Network error. Please check your connection.';
      showToast('✖️ Message failed (network)', true);
    }
  ...
  try {
    ...
    if (data.success) {
      msg.className = 'mt-3 p-3 rounded text-sm text-center bg-[#00ff9c]/10 border border-[#00ff9c]/30 text-[#00ff9c]';
      msg.textContent = data.message || '✅ Message received! We will contact you soon.';
      form.reset();
      showToast('✅ Message sent!');
    } else {
      msg.className = 'mt-3 p-3 rounded text-sm text-center bg-[#ff2b2b]/10 border border-[#ff2b2b]/30 text-[#ff2b2b]';
      msg.textContent = data.message || 'Failed to send message. Please call us directly.';
      showToast('✖️ Failed to send message', true);
    }
  } catch (err) {
    msg.className = 'mt-3 p-3 rounded text-sm text-center bg-[#ff2b2b]/10 border border-[#ff2b2b]/30 text-[#ff2b2b]';
    msg.textContent = 'Server unavailable. Please call: +91 99375 82815';
    showToast('✖️ Message failed (server offline)', true);
  }
  msg.classList.remove('hidden');
  btn.disabled = false;
});
...
(async () => {
  ...
  // Bind preview toggle
  document.getElementById('toggleMenuBtn')?.addEventListener('click', toggleMenuExpansion);
})();
