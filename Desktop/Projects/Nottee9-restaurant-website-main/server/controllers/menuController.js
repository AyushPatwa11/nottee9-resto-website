...
const toggleOffer = async (req, res) => {
  try {
    const { offerType, offerValue } = req.body; // offerType: 'percent'|'fixed', offerValue: number
    const item = await Menu.findById(req.params.id);
    if (!item) return res.status(404).json({ success: false, message: 'Dish not found' });

    // Toggle: if currently not an offer and request contains offer details, enable with computed price
    if (!item.isOffer) {
      // Validate inputs
      if (!offerType || typeof offerValue === 'undefined' || offerValue === null || isNaN(offerValue)) {
        return res.status(400).json({ success: false, message: 'offerType and offerValue are required to enable offer' });
      }
      const val = Number(offerValue);
      let computedPrice = null;
      if (offerType === 'percent') {
        if (val <= 0 || val >= 100) return res.status(400).json({ success: false, message: 'Percent must be between 0 and 100' });
        computedPrice = Math.max(0, Math.round(item.price * (1 - val / 100)));
      } else if (offerType === 'fixed') {
        if (val <= 0 || val >= item.price) return res.status(400).json({ success: false, message: 'Fixed discount must be greater than 0 and less than price' });
        computedPrice = Math.max(0, Math.round(item.price - val));
      } else {
      ...
      item.offerValue = val;
      item.offerPrice = computedPrice;
      await item.save();
      return res.json({ success: true, data: item, message: `Offer activated for ${item.name}` });
    }

    // If currently an offer, disable it (no need for details)
    item.isOffer = false;
    item.offerType = 'percent';
    item.offerValue = null;
    item.offerPrice = null;
    await item.save();
    res.json({ success: true, data: item, message: `Offer deactivated for ${item.name}` });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
