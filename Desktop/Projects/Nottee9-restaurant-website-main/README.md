...
The site can send contact form messages to the restaurant owner. Two options are supported:

- SendGrid (recommended): Set `SENDGRID_API_KEY` and optionally `CONTACT_RECIPIENT` in the server `.env` file.
- If `SENDGRID_API_KEY` is not set, the server will log the contact message to the console (safe fallback for local/dev).
...
CONTACT_RECIPIENT=hello@nottee9.in
CONTACT_SENDER=no-reply@nottee9.in
```

Alternatively you can use a zero-server form provider (Formspree/Getform) by changing the front-end form `action`.

Formspree quick setup (zero-server):

1. Sign up at https://formspree.io and create a form. You'll get a form endpoint like `https://formspree.io/f/{form_id}`.
2. Open `client/index.html` and add this near the top (replace with your form id):

```html
<script>window.FORM_PROVIDER_URL = 'https://formspree.io/f/your_form_id_here'</script>
```

3. The site will automatically send contact form submissions to that URL (no server changes needed).

Notes:
- Use `Accept: application/json` (our frontend does this) so the provider returns JSON responses.
- Formspree has spam protection and email forwarding; check their dashboard to configure recipient (you can forward to hello@nottee9.in).
